...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(item => {
    let secondSpace = item.indexOf(' ', 3);
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(item => {
    ...
    let ingredients = newArr[2];
    for (let i = 3; i < newArr.length; i ++) {
      ingredients += ` ${newArr[i]}`;
    }
...
const removeLastCharacters = (str, numberOfCharacters) => {
  // Solution code here...
  return (numberOfCharacters > str.length ? '' : numberOfCharacters < 0 ? str : str.substring(0, str.length - numberOfCharacters));
};
