...
const evenOdd = (arr) => {
  ...
  let arr2 = [];
  arr.map(item => {
    if (/^\d+$/.test(item) === false){arr2.push('N/A');}
    else if (item % 2 === 0){ arr2.push('even');}
    else{arr2.push('odd');}
  });
  return arr2;
};
...
const extractStats = (arr) => {
  ...
  arr.map(item => {
    let object = {};
    let total = item.effort + item.baseStat;
    object.name = (item.stat.name);
    object.total = (total);
    arr2.push(object);
  });
  return arr2;
